...
We take the `kwdyz11.arrow` dataset [@Kliegl2011] from an experiment looking at three effects of visual cueing under four different cue-target relations (CTRs).
...
```{julia}
#| code-fold: true
dat = @chain dataset(:kwdyz11) begin
  DataFrame
  select(
    :subj =>
