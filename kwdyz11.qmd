...
We specify three contrasts for the four-level factor `tar` that are derived from spatial, object-based, and attractor-like features of attention.
...
```{julia}
dat_subj = combine(
  groupby(dat, [:subj, :tar]),
  :rt => length => :n,
  :rt => mean => :rt_m,
  :rt => (r -> mean(log, r)) => :lrt_m,
)
dat_subj.tar = categorical(dat_subj.tar, levels=levels(dat.tar))
dat_subj
```
...
```{julia}
contrasts = Dict(
  :tar => SeqDiffCoding(; levels=["val", "sod", "dos", "dod"]),
  :subj => Grouping(),
)
m1 = let
  form = @formula(log(rt) ~ 1 + tar + (1 + tar | subj))
...
The upper limit of the interval for the critical CP `tar: sod, tar: dod` is hitting the upper wall of a perfect correlation.
...
labels = [
  "tar: sod" => "spatial effect",
  "tar: dos" => "object effect",
  "tar: dod" => "attraction effect",
...
let
  labels = [
    "(Intercept), tar: sod" => "GM, spatial",
    "(Intercept), tar: dos" => "GM, object",
    "tar: sod, tar: dos" => "spatial, object",
    "(Intercept), tar: dod" => "GM, attraction",
    "tar: sod, tar: dod" => "spatial, attraction",
    "tar: dos, tar: dod" => "object, attraction",
  ]
  draw(
    data(tbl) *
    mapping(
